...
// ---- compat ExpressLRS 3.6.4 -----------------------------------------------
// Ver a declaração em LR2021.h. A banda sai da própria frequência — é o mesmo
// critério que o Config nativo usa internamente (regfreq < 1 GHz), então os
// dois caminhos concordam. Em modo dual o tx_main chama isto duas vezes, uma
// por banda, e cada chamada se resolve pela frequência que recebeu.
void LR2021Driver::Config(const uint8_t bw, const uint8_t sf, const uint8_t cr, const uint32_t freq,
                          const uint8_t PreambleLength, const bool InvertIQ, const uint8_t PayloadLength,
                          const bool setFSKModulation, const uint8_t fskSyncWord1, const uint8_t fskSyncWord2,
                          const SX12XX_Radio_Number_t radioNumber)
{
    const bool isSubGHz = freq < 1000000000;
    const RadioBandMod::Combined modulation =
        setFSKModulation ? (isSubGHz ? RadioBandMod::GFSK_900 : RadioBandMod::GFSK_2G4)
                         : (isSubGHz ? RadioBandMod::LORA_900 : RadioBandMod::LORA_2G4);

    Config(bw, sf, cr, freq, PreambleLength, InvertIQ, PayloadLength, modulation,
           fskSyncWord1, fskSyncWord2, 0, 0, radioNumber);
}
